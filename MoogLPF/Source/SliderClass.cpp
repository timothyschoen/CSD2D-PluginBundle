...
void SliderClass::makeSliders(AudioProcessorEditor &process, NewProjectAudioProcessor &processor)
{
  ...
  sliders[1].setTextValueSuffix (" Cutoff");
  ...
  process.getLookAndFeel().setColour (Slider::thumbColourId, Colour::fromString("FFffd33c"));
...
void SliderClass::draw(Graphics &g, AudioProcessorEditor &process)
{
   ...
   g.drawFittedText ("Cutoff", 282.5, 215, 50, 20, Justification::centred, 1);
  ...
  g.setColour (Colour::fromString("FFffd33c"));
  g.fillRect (process.getWidth()-75, 0, 75, process.getHeight());
  g.setFont (customFont);
  g.drawSingleLineText ("MoogLPF ", 25, 75);
